...
$service = getArrayVal($_POST, "service");
...
$designation = getArrayVal($_POST, "designation");
$gradelevel = getArrayVal($_POST, "gradelevel");
...
} elseif ($action == "adduser")
{
	...
	$newid = $user->add($name, $email, $company, $pass, $sysloc, $tags, $rate, $designation, $gradelevel, $employmentdate, $dob);
    if ($newid)
    {
        if (!empty($assignto))
        {
...
} elseif ($action == "edituser")
{
    ...
    if (!empty($_FILES['userfile']['name']))
    {
        ...
        if ($user->edit($id, $name, "" , $email, $tel1, $tel2, $company, $zip, $gender, $turl, $address1, $address2, $state, $country, $tags, $locale, $avatar, $rate, $designation, $gradelevel, $employmentdate, $dob, $disengagementdate, $exammonitor))
        {
    ...
    {
        if ($user->edit($id, $name, "", $email, $tel1, $tel2, $company, $zip, $gender, $turl, $address1, $address2, $state, $country, $tags, $locale, "", $rate, $designation, $gradelevel, $employmentdate, $dob, $disengagementdate, $exammonitor))
        {
            if (!empty($newpass) and !empty($repeatpass))
            {
                $user->admin_editpass($id, $newpass, $repeatpass);
...
elseif ($action == "addpro")
{
    ...
    $add = $project->add($name, $desc, $end, $budget, 0, $service, $company);
    if ($add)
    {
