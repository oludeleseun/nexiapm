...
class timetracker
{
     ...
     */
    function add($user, $project, $task, $comment, $started, $ended, $logday = "", $estimatedHours)
    {
        ...
        $ins = mysql_query("INSERT INTO timetracker (user,project,task,comment,started,ended,hours,pstatus,estimatedHours) VALUES ($user,$project,$task,'$comment','$started','$ended','$hours',0,'$estimatedHours')");
     ...
     * @return bool
     */
    function edit($id, $task, $comment, $started, $ended, $estimatedHours)
    {
        $comment = mysql_real_escape_string($comment);
        $started = (int) $started;
        $ended = (int) $ended;
        ...
        $upd = mysql_query("UPDATE timetracker SET task='$task',comment='$comment',started='$started',ended='$ended',hours='$hours',estimatedHours='$estimatedHours' WHERE ID = $id");
        if ($upd)
        {
            return true;
        }
    ...
    function getUserTrack($user, $project = 0, $task = 0, $start = 0, $end = 0 , $lim = 50)
    {
        ...
        if (isset($sel))
        {
            while ($data = @mysql_fetch_array($sel))
            {
                ...
                $pname = mysql_query("SELECT name, service, budget FROM projekte WHERE ID = $data[project]");
                $pdata = mysql_fetch_row($pname);
                $pname = stripslashes($pdata[0]);
				$pservice = stripslashes($pdata[1]);
				$pbudget = stripslashes($pdata[2]);
    ...
    function getProjectTrack($project, $user = 0, $task = 0, $start = 0, $end = 0, $lim = 50)
    {
        ...
        if (isset($sel))
        {
            while ($data = @mysql_fetch_array($sel))
            {
				...
				$urate = mysql_query("SELECT rate FROM user WHERE ID = $data[user]");
                $urate = mysql_fetch_row($urate);
				$urate = stripslashes($urate[0]);
