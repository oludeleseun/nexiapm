...
class project {
     ...
     */
    function add($name, $desc, $end, $budget, $assignme = 0, $service, $company)
    {
        ...
        $ins1 = mysql_query("INSERT INTO projekte (`name`, `desc`, `end`, `start`, `status`, `budget`, `service`, `company`) VALUES ('$name','$desc','$end','$now',1,'$budget', '$service', '$company')");
     ...
     * @return int $insid ID des neu angelegten Projekts
     */
    function AddFromBasecamp($name, $desc, $start, $status = 1, $company)
    {
        $name = mysql_real_escape_string($name);
        $desc = mysql_real_escape_string($desc);
        ...
        $ins1 = mysql_query("INSERT INTO projekte (`name`, `desc`,`end`, `start`, `status`, `company`) VALUES ('$name','$desc','$now','$start','$status','$company')");
     ...
     * @return bool
     */
    function edit($id, $name, $desc, $end, $budget, $service, $company)
    {
        ...
        $upd = mysql_query("UPDATE projekte SET name='$name',`desc`='$desc',`end`='$end',budget=$budget, `service`='$service', `company`='$company' WHERE ID = $id");
    ...
    function getProject($id)
    {
        ...
        if (!empty($project)) {
			...
			$cname = mysql_query("SELECT name FROM company WHERE ID = $project[company]");
            $cname = mysql_fetch_row($cname);
            $cname = stripslashes($cname[0]);
			$project["companyname"] = $cname;
