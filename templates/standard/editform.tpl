...
<div class="block_in_wrapper">
    ...
    <div class="row">
    	<label for="budget">{#service#}:</label><input type="text" class="text" name="service" id="service" required="0" realname="{#service#}" value = "{$project.service}" />
    </div>
    ...
    <div class="row">
        ...
        <select name = "company" realname = "{#company#}" required="1" exclude = "-1" id = "company">
            ...
            {section name = company loop=$companies}
                <option value = "{$companies[company].ID}" id="role{$companies[company].ID}" {if $companies[company].ID == $project.company}selected="selected"{/if}>{$companies[company].name}</option>
            {/section}
        </select>
    </div>
